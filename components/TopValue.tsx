import { type Studio, lowestPerClass } from "@/lib/types";
...
/** Compact "best value" leaderboard — cheapest price per class, excludes trials & memberships. */
export default function TopValue({ studios }: { studios: Studio[] }) {
  const ranked = studios
    .map((s) => ({ s, pc: lowestPerClass(s) }))
  ...
  return (
    <section className="relative mb-8 rounded-lg border-2 border-line bg-panel p-5">
      ...
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-[11px] font-semibold uppercase tracking-wider text-muted">lowest price / class</span>
      </div>
      <p className="mt-1 text-xs text-muted">Cheapest committed rate per studio — excludes trials and memberships.</p>
          ...
          <li key={s.id}>
            ...
            >
              ...
              <span className="shrink-0 text-right">
                ...
                <span className="text-[11px] text-muted">/class</span>
              </span>
            </a>
          </li>
        ))}
